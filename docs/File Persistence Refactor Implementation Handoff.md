...
Updated: 2026-09-12. Status: checkpoint 5b is accepted. The maintainer confirmed the revised clean **Open in new window** path works correctly after migration began awaiting the exact active `setViewData()` completion rather than relying on diagnostic timing. Earlier instrumented coverage also passed sustained large-scene synchronization, same-file editing, mixed Markdown/Excalidraw views, both popout operations, dirty migration, and an unsaved switch to Markdown mode. Temporary diagnostics remain removed. Commit checkpoint 5b, then begin original checkpoint 6 with ordinary dirty unload as its first independently testable increment.
...
| 5b.5 | Enable matching at the coalesced read boundary | Complete; clean runtime accepted; commit pending | Boolean-armed Markdown events now continue through the existing pending latest-state synchronization loop. Its single owned Vault read revalidates runtime/target identity and skips parse/publication only for exact successful-write or accepted-content matches; all other classifications process normally. Raw `.excalidraw` suppression and same-file edit ownership remain unchanged. Explicit `setViewData()` completion ownership fixes the diagnostic-sensitive **Open in new window** lifecycle race. Maintainer accepted the revised clean build |
| 6 | Detached persistence ownership and ordering | Next | Start with ordinary dirty unload only: data-only handoff, explicit outcomes, and ordering against live writes. Migrate special window handoff only after this increment passes |
