...
Updated: 2026-09-13. Status: checkpoint 5b is accepted and committed as `31592829`. Checkpoint 6a ordinary dirty-unload handoff is accepted and committed as `178d3178`. Checkpoint 6b.1 physical per-path write exclusion is now maintainer-accepted after its large-scene concurrent save/close, rapid dirty-close, and popout runtime gate passed. Its commit is pending. Popout-to-main persistence still uses the specialized replacement handoff and remains the next checkpoint 6 increment.
...
| 6 | Detached persistence ownership and ordering | 6a committed; 6b.1 accepted, commit pending; migration consolidation remains | Ordinary dirty unload hands immutable prepared text to a plugin-owned FIFO and reports `persistence-handed-off`; commit `178d3178`. Data-only leases now prevent live `super.save()`, direct main-realm migration writes, and ordinary detached requests from physically overlapping or overtaking accepted work. Maintainer runtime tests passed, including the two independently dirty close case. Popout-to-main replacement persistence still uses its specialized handoff and is not yet consolidated |
