...
Updated: 2026-09-12. Status: baseline comparison completed; the maintainer confirms the isolated `NEVER` change resolves dirty ping-pong, and that change plus the planning documents is committed as `45144b52`. Two maintainer traces established separate causes for the remaining data loss/divergence. The validated 5b.2 dirty-receiver routing correction is committed as `58d8ea84`. Checkpoint 5b.3 minimum save ownership is implemented, statically verified, and accepted by the maintainer after large-scene compression, nested/bound-element, and persistence testing; its clean checkpoint commit is next. The remaining on-screen divergence is still expected until 5b.5 replaces cross-view post-save Boolean suppression. The broad 5b.1 instrumentation has been removed; 5b.4 will add only observation-specific diagnostics.
...
| 5b.1 | Trace first missed propagation, with dirty-origin tracing if needed | Evidence complete | Trace A: dirty V2 was routed through full reload and its edit was cleared. Trace B: six modify events were cross-suppressed by the receiver's stale post-save Boolean; both views finally reported clean/idle with different scene fingerprints. Temporary prefix `EXCALIDRAW_PERSISTENCE_5B`; instrumentation remains intentionally uncommitted |
...
| 5b.3 / 9a | Minimum immutable save ownership | Accepted; clean commit next | Save intent now carries producer/target-generation/operation/requested-revision identity. Save-owned scene, nested element data, app-state subset, files, deletions, selected IDs, and source envelope are detached before normalization; `PreparedSave` owns the exact final text and actual captured revision. Only physical successful write outcomes publish a successful prepared identity. Focused checks/builds and maintainer large-scene runtime tests pass; suppression behavior is intentionally unchanged |
