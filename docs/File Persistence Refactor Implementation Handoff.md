...
Updated: 2026-09-12. Status: baseline comparison completed; the maintainer confirms the isolated `NEVER` change resolves dirty ping-pong, and that change plus the planning documents is committed as `45144b52`. Two maintainer traces established separate causes for the remaining data loss/divergence. The validated 5b.2 dirty-receiver routing correction is committed as `58d8ea84`. The maintainer accepted checkpoint 5b.3 minimum save ownership after large-scene compression, nested/bound-element, and persistence testing; its clean commit is `071e3ef3`. The remaining on-screen divergence is still expected until 5b.5 replaces cross-view post-save Boolean suppression. The broad 5b.1 instrumentation has been removed; 5b.4 will add only observation-specific diagnostics.
