...
## 警告
这是一个使用最新未发布的 Excalidraw 0.7.0 包的预发布版本。根据我的测试经验，这个版本是稳定的，但在 Excalidraw 0.7.0 正式发布之前还不能用于生产环境。

## 修复
### Excalidraw 0.7.0 修复
这些 Excalidraw 中的问题影响了 Excalidraw 在 Obsidian 中的可用性，具体是：
- 影响 Obsidian 其他工作区叶片的键盘事件传播问题。参见错误 [#11](https://github.com/zsviczian/obsidian-excalidraw-plugin/issues/11) 和 [#12](https://github.com/zsviczian/obsidian-excalidraw-plugin/issues/12)。
- 解决了工作区叶片标题上的 ctrl+x 问题
- 库项目放置在视图外的问题
- 从粘贴数据生成的图表放置问题

### 应用逻辑修复
- 绘图定位问题也已修复。现在打开时，绘图应该始终在工作区画布中居中显示。
- 修复了关闭绘图时，如果不切换到文件视图中的另一个文档，绘图不会保存的问题。
- 绘图始终使用保存时的主题进行初始化。
- 将模板库文件移至 data.json，希望未来 Obsidian 同步能够覆盖这些内容。（抱歉，这可能会再次影响使用了之前测试版本的用户的模板库）
- main.js 文件大小大幅减小（特别是与第一个测试版本相比...从 12MB 减少到 560kb）

## 重大变更
Excalidraw 0.7.0 还对客户端模板库的处理方式引入了重大变更。因此，您的模板库中的项目（但不是保存在您的 vault 中的绘图！）将会丢失。您需要重新添加库项目。
![image](https://user-images.githubusercontent.com/14358394/115918236-96067080-a477-11eb-8daa-836a68719f81.png)

## 安装方法：
将 3 个文件 main.js、manifest.json、styles.css 复制到 vault/.obsidian/plugins/obsidian-excalidraw-plugin/ 文件夹。
![image](https://user-images.githubusercontent.com/14358394/115394105-b8339080-a1e2-11eb-8395-ef42777e031e.png)
...
## 修复
Excalidraw 0.7.0 修复了两个影响 Obsidian 中 Excalidraw 可用性的错误：
- 影响 Obsidian 其他工作区叶片的键盘事件传播问题。参见错误 [#11](https://github.com/zsviczian/obsidian-excalidraw-plugin/issues/11) 和 [#12](https://github.com/zsviczian/obsidian-excalidraw-plugin/issues/12)。
- 库项目放置在视图外的问题。
...
## 警告
这是使用未发布的 Excalidraw 0.7.0 包的测试版本。由于 Excalidraw 0.6.0 和 0.7.0 之间的重大变更，这个版本会破坏库。您的 Excalidraw 中的库项目可能会永久丢失！！！但您的 Obsidian vault 不会受到影响。
修复了 [#12](https://github.com/zsviczian/obsidian-excalidraw-plugin/issues/12) 和 [#11](https://github.com/zsviczian/obsidian-excalidraw-plugin/issues/11)

## 修复
修复了影响 Obsidian 其他工作区叶片的键盘事件传播问题。
...
## 发布说明
### 新功能
- 使用 [[image.excalidraw|100]] 或 [[image.excalidraw|100x100]] 格式调整嵌入图片大小。解决问题 [#4](https://github.com/zsviczian/obsidian-excalidraw-plugin/issues/4)

### 修复
- 深色模式 [#7](https://github.com/zsviczian/obsidian-excalidraw-plugin/issues/7)
- 应用图标 [#6](https://github.com/zsviczian/obsidian-excalidraw-plugin/issues/6)
- 应用描述 [#5](https://github.com/zsviczian/obsidian-excalidraw-plugin/issues/5)
...
### iPad安装
使用"Filebrowser"应用程序,该应用可以显示隐藏文件夹。在 .obsidian 中创建一个文件夹,并从此仓库复制3个文件。这样就可以工作了,虽然有一个已报告的问题需要解决,这将在下一个 Obsidian 版本中解决。
...
## 发布说明
支持嵌入(transclusion)。
使用命令面板: Excalidraw: 在 markdown 文档中插入 .excalidraw 文件链接
查看[项目页面](https://github.com/zsviczian/obsidian-excalidraw-plugin)获取完整功能列表和演示视频
...
发布说明:
主要变更:现已集成到 Obsidian 文件浏览器中。
下一步是嵌入功能!
插件可以工作,但我还没有解决在 .md 文件中嵌入的问题。
