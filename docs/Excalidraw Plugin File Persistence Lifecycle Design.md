...
Checkpoint 5b is accepted and committed as `31592829`. Checkpoint 6a is implemented and maintainer-accepted for ordinary dirty unload only. Immutable prepared text transfers to a plugin-owned per-path FIFO that resolves the file at execution, refuses missing/recreated targets and empty payloads, contains failures, and does not coalesce independent producer snapshots. The former `view-unload-scheduled` result is now `persistence-handed-off`, which names responsibility transfer without claiming physical completion. Aggressive acceptance covered five simultaneous mixed Markdown/Excalidraw representations, side-by-side views, both popout operations, desktop/mobile concurrent edits, and an Obsidian Sync malformed-input recovery. Normal live `super.save()`, popout migration persistence, BAK behavior, and their cross-producer ordering remain unchanged; those are checkpoint 6b/7 work. See the handoff's checkpoint 6a implementation record and gate.
