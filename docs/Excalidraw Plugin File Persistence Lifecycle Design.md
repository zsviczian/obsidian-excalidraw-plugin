...
Checkpoint 5b is accepted and committed as `31592829`; checkpoint 6a is accepted and committed as `178d3178`. Immutable prepared text transfers to a plugin-owned per-path FIFO that resolves the file at execution, refuses missing/recreated targets and empty payloads, contains failures, and does not coalesce independent producer snapshots. Checkpoint 6b.1 shares that physical path boundary with live `super.save()` and the direct main-realm migration write through data-only leases, and its large-scene concurrent save/close, rapid dirty-close, and popout runtime gate is maintainer-accepted. Popout-to-main replacement persistence handoff plus BAK remain unchanged. See the handoff's checkpoint 6b.1 acceptance and next checkpoint 6 increment.
