...
Checkpoint 5b is accepted and committed as `31592829`; checkpoint 6a is accepted and committed as `178d3178`; checkpoint 6b.1 is accepted and committed as `e5ba009c`. Immutable prepared text transfers to a plugin-owned per-path FIFO that resolves the file at execution, refuses missing/recreated targets and empty payloads, contains failures, and does not coalesce independent producer snapshots. Checkpoint 6b.2 folds popout-to-main serialized handoff storage and per-path ordering into that queue while preserving the established rule that the recreated main-window view initiates the physical write; its maintainer runtime gate passed. Checkpoint 6 is complete. Drawing-runtime migration handoff remains separate, and BAK remains checkpoint 7, which has not started.
