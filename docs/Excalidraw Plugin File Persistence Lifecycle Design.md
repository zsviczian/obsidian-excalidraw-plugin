...
Checkpoint 5b is accepted; original checkpoint 6 is next. Commit `45144b52` contains the confirmed `NEVER` correction, `58d8ea84` contains the validated 5b.2 dirty-receiver route, and `071e3ef3` contains the accepted minimum immutable save ownership. Exact-content traces validated the enabling distinction: own echoes match successful bytes, while cross-view writes consumed by stale Boolean suppression contain different bytes. Popout operations are not interchangeable: **Move to new window** remained healthy, while **Open in new window** exposed migration during the new leaf's active view-data load. Migration now captures the exact active `setViewData()` completion before synchronous source unmount, awaits that completion after any save flush, then yields one destination task before source replacement. The maintainer confirmed the revised clean path works correctly. Temporary diagnostics remain absent. Begin checkpoint 6 with ordinary dirty unload only; do not collapse the special migration persistence handoff until the general queue's ownership and ordering are independently validated. See the handoff's 5b.2a–5b.5 records and checkpoint 6 specification for evidence and gates.
