...
export class EmbeddableSettings extends Modal {
  ...
  async createForm() {
    ...
    if (this.file) {
      new Setting(this.contentEl).setName(t("ES_RENAME")).addText((text) =>
        text
    ...
    const zoomValue = (): DocumentFragment => {
      return fragWithHTML(
        `${t("ES_ZOOM_100_RELATIVE_DESC")}<br>Current zoom is <b>${Math.round(this.zoomValue * 100)}%</b>`,
      );
    };
    ...
    const zoomSetting = new Setting(this.contentEl)
      ...
      .setDesc(zoomValue())
      .addButton((button) =>
        button.setButtonText(t("ES_ZOOM_100")).onClick(() => {
    ...
    if (this.isYouTube) {
      new Setting(this.contentEl)
        ...
        .addText((text) =>
          text.setValue(this.youtubeStart).onChange(async (value) => {
  ...
  private async applySettings() {
    ...
    const el = this.ea.getElement(
      this.element.id,
    ) as Mutable<ExcalidrawEmbeddableElement>;
        ...
        if (this.app.vault.getAbstractFileByPath(newPath)) {
          ...
          try {
            ...
            el.link = this.element.link.replace(
              /(\[\[)([^#\]]*)([^\]]*]])/,
              `$1${this.plugin.app.metadataCache.fileToLinktext(
                this.file,
                this.view.file.path,
                true,
              )}$3`,
    ...
    if (
      this.isMDFile &&
      (this.mdCustomData.backgroundColor !==
        this.element.customData?.backgroundColor ||
        this.mdCustomData.borderColor !==
          this.element.customData?.borderColor ||
        this.mdCustomData.backgroundOpacity !==
