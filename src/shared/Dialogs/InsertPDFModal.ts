...
export class InsertPDFModal extends Modal {
  ...
  private async loadPageDimensions(pages?: number[]) {
    ...
    for (const pageNum of pagesToLoad) {
      ...
      try {
        ...
        this.pageDimensionsByPage.set(pageNum, {
  ...
  async createForm() {
    ...
    const numPagesMessages = () => {
      ...
      setSanitizedHtml(
        numPagesMessage,
        `There are <b>${numPages}</b> pages in the selected document.`,
      );
    };
    ...
    const rangeOnChange = async (value: string) => {
      ...
      if (pages.length > 15) {
        setSanitizedHtml(
          importPagesMessage,
          `You are importing <b>${pages.length}</b> pages. ⚠️ This may take a while. ⚠️`,
        );
      } else {
        setSanitizedHtml(
          importPagesMessage,
          `You are importing <b>${pages.length}</b> pages.`,
        );
      }
    ...
    const suggester = new FileSuggestionModal(
      ...
      this.app.vault
        .getFiles()
        .filter((f: TFile) => f.extension.toLowerCase() === "pdf"),
      this.plugin,
    );
    ...
    new Setting(ce)
      ...
      .addToggle((toggle) => {
        bbToggle = toggle;
        toggle.setValue(this.borderBox).onChange((value) => {
    ...
    new Setting(ce)
      ...
      .addToggle((toggle) => {
        fToggle = toggle;
        toggle.setValue(this.frame).onChange((value) => {
          this.frame = value;
          if (value) {
    ...
    new Setting(ce)
      ...
      .addToggle((toggle) =>
        toggle.setValue(this.groupPages).onChange((value) => {
    ...
    new Setting(ce)
      ...
      .addToggle((toggle) => {
        laiToggle = toggle;
        toggle.setValue(this.lockAfterImport).onChange((value) => {
    ...
    new Setting(ce).setName("Import direction").addDropdown((dropdown) =>
      dropdown
        .addOptions({
          down: "Top > Down",
          right: "Left > Right",
    ...
    const importSizeSetting = new Setting(ce)
      ...
      .setDesc(
        `${Math.round(initialDims.width * this.importScale)} x ${Math.round(initialDims.height * this.importScale)}`,
      )
      .addSlider((slider) =>
        slider
        ...
        button
          ...
          .onClick(async () => {
            ...
            const cellWidth = Math.round(
              maxPageDimensions.width * this.importScale,
            );
            const cellHeight = Math.round(
              maxPageDimensions.height * this.importScale,
            );
            ...
            for (let i = 0; i < this.pagesToImport.length; i++) {
              ...
              const dims =
                this.pageDimensionsByPage.get(page) ?? maxPageDimensions;
              ...
              const topX = Math.round(
                cellWidth * column + this.gapSize * column,
              );
