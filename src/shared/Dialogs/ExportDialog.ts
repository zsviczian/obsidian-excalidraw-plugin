...
export class ExportDialog extends Modal {
  ...
  updateBoundingBox() {
    if (this.isSelectedOnly) {
      this.boundingBox = this.ea.getBoundingBox(
        this.view.getViewSelectedElements(),
      );
    } else {
      this.boundingBox = this.ea.getBoundingBox(this.ea.getViewElements());
    }
  ...
  createForm() {
    ...
    this.buttonContainerRow1 = this.contentEl.createDiv({
  ...
  private size(): DocumentFragment {
    const width = Math.round(
      this.scale * this.boundingBox.width + this.padding * 2,
    );
    const height = Math.round(
      this.scale * this.boundingBox.height + this.padding * 2,
    );
    return fragWithHTML(
      `${t("EXPORTDIALOG_SIZE_DESC")}<br>${t("EXPORTDIALOG_SCALE_VALUE")} <b>${this.scale}</b><br>${t("EXPORTDIALOG_IMAGE_SIZE")} <b>${width}x${height}</b>`,
    );
  }
    ...
    switch (this.activeTab) {
      ...
      case "screenshot":
        ...
        if (this.view.isInMainObsidianWorkspace) {
          ...
          this.contentContainer.createEl("p", {
            text: t("EXPORTDIALOG_NOT_AVAILALBE"),
          });
        }
        break;
      case "image":
      default:
        this.contentContainer.createEl("h1", {
          text: t("EXPORTDIALOG_IMAGE_SETTINGS"),
        });
        this.contentContainer.createEl("p", {
          text: t("EXPORTDIALOG_IMAGE_DESC"),
        });
        break;
    }
  }
  ...
  private createImageSettings(isScreenshot: boolean = false) {
    ...
    const padding = (): DocumentFragment => {
      return fragWithHTML(
        `${t("EXPORTDIALOG_CURRENT_PADDING")} <b>${this.padding}</b>`,
      );
    };
    ...
    if (!isScreenshot) {
      ...
      new Setting(this.contentContainer)
        ...
        .addToggle((toggle) =>
          toggle.setValue(this.exportInternalLinks).onChange((value) => {
  ...
  private createPDFSettings() {
    ...
    new PDFExportSettingsComponent(this.contentContainer, pdfSettings, () => {
  ...
  private createPDFButton() {
    const bSavePDFSettings = this.buttonContainerRow1.createEl("button", {
      text: t("EXPORTDIALOG_SAVE_PDF_SETTINGS"),
      cls: "excalidraw-export-button",
    });
  ...
  public getPaperColor(): string {
    switch (this.paperColor) {
      case "white":
        return this.theme === "light" ? "#ffffff" : "#000000";
      case "scene":
        return this.api.getAppState().viewBackgroundColor;
      case "custom":
        return this.customPaperColor;
      default:
        return "#ffffff";
    }
  }
