...
## New
- On Obsidian 1.13 and newer, Excalidraw settings use Obsidian's searchable multi-page interface by default. A top-level setting lets you switch between this interface and the legacy single-page layout after restarting Obsidian. Embed/export, image-cache, PDF, embedded-file, Markdown-image, custom-pen, font, miscellaneous, automation, installed-script, compatibility, and AI provider/model options are included in the shared searchable hierarchy and remain grouped the same way in the legacy layout.
- Settings registered by installed Excalidraw Automate scripts have moved from the bottom of the settings tab to **Excalidraw Automate → Settings for installed Scripts**. The page appears when an installed script exposes at least one visible setting and updates when a script saves new settings while the tab is open.
