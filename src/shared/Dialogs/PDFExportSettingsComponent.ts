...
export class PDFExportSettingsComponent {
  ...
  isOrientationAndTilingVisible() {
    return !(
      this.settings.pageSize === "HD Screen" ||
      this.settings.pageSize === "MATCH IMAGE"
    );
  }

  render() {
    const pageSizeOptions: Record<string, string> = Object.keys(
      STANDARD_PAGE_SIZES,
    ).reduce(
      (acc, key) => ({
        ...acc,
    ...
    new Setting(div)
      ...
      .addDropdown((dropdown) =>
        dropdown
          ...
          .setValue(
            this.settings.fitToPage === 1
              ? "fit"
              : typeof this.settings.fitToPage === "number"
                ? `fit-${this.settings.fitToPage}`
                : "scale",
          )
          .onChange((value) => {
            this.settings.fitToPage =
              value === "scale"
                ? 0
                : value === "fit"
                  ? 1
