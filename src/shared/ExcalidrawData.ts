...
export class ExcalidrawData {
  ...
  public async loadData(
    ...
    const fallbackMatch = normalMatch
      ? null
      : data.match(/(.*##? Text Elements(?:\n|$))/m);

    if (!normalMatch && !fallbackMatch) {
      // "# Excalidraw Data" (or equivalent) was found above, so `position`
      // was not -1 and we got here, but there is no "## Text Elements"
      // heading anywhere in the remaining data. This is a valid state for
      // a file with zero text elements (e.g. hand-authored or generated by
      // external tooling) — treat it the same as the `position === -1`
      // guard above rather than crashing on a null match.
    ...
    const textElementsMatch = normalMatch ? normalMatch[0] : fallbackMatch[0];
