...
export default {
  ...
  AI_PROVIDER_BASE_URL_EMPTY: "由提供商类型推导",
  ...
  AI_PROVIDER_API_KEY_DESC:
    "共享 AI 层使用的主 API key。除非输入框获得焦点，否则该值会被隐藏。",
  AI_PROVIDER_API_KEY_PLACEHOLDER: "提供商 API key",
  AI_PROVIDER_BASE_URL_NAME: "AI 基础 URL",
  AI_PROVIDER_BASE_URL_DESC: `可选的提供商基础 URL。可用于本地 OpenAI 兼容服务器或自托管网关。您既可以填写类似 https${+"://api.openai"}${+".com/v1 的基础 URL，也可以填写类似 https"}${+"://api.openai"}${+".com/v1/chat/completions 的完整聊天 endpoint；Excalidraw 会自动规范化已知的 OpenAI 风格 endpoint。"}`,
  ...
  AI_PROVIDER_IMAGE_GENERATION_ENDPOINT_DESC:
    "可选的完整 endpoint 覆盖，用于图像生成请求。除非您的提供商使用非标准路径，否则请留空。",
  AI_PROVIDER_IMAGE_GENERATION_ENDPOINT_PLACEHOLDER:
    "可选的完整图像生成 endpoint",
  AI_PROVIDER_IMAGE_EDITS_ENDPOINT_NAME: "AI 图像编辑 endpoint 覆盖",
  AI_PROVIDER_IMAGE_EDITS_ENDPOINT_DESC:
    "可选的完整 endpoint 覆盖，用于图像编辑请求。除非您的提供商使用非标准路径，否则请留空。",
  AI_PROVIDER_IMAGE_EDITS_ENDPOINT_PLACEHOLDER: "可选的完整图像编辑 endpoint",
  AI_PROVIDER_IMAGE_VARIATIONS_ENDPOINT_NAME: "AI 图像变体 endpoint 覆盖",
  ...
  AI_PROVIDER_DEFAULT_TEXT_MODEL_PLACEHOLDER:
    "例如：gpt-5-mini、claude-sonnet、gemini-2.5-pro",
  AI_PROVIDER_DEFAULT_VISION_MODEL_NAME: "视觉模型",
  AI_PROVIDER_DEFAULT_VISION_MODEL_DESC:
    "用于 diagram-to-code 或图文混合提示等图像理解任务的模型。<br>提供商：{{provider}}（{{providerType}}）<br>API key：{{apiKey}}<br>模型名称：{{model}}<br>Endpoint：{{endpoint}}",
  AI_PROVIDER_DEFAULT_VISION_MODEL_PLACEHOLDER:
    "例如：gpt-5-mini、claude-sonnet、gemini-2.5-flash",
  ...
  PREFER_COMPACT_MODE_DESKTOP_DESC:
    "切换托盘模式时，开启该项，Excalidraw 将在紧凑模式与托盘模式之间切换。" +
    "关闭该项，则在完整模式与托盘模式之间切换。",
  COMPACT_MODE_NAME: "在平板端偏好紧凑模式",
  COMPACT_MODE_DESC:
    "该项允许平板端和桌面端设置不同的默认模式。你可以在平板端默认使用紧凑模式，而在桌面端默认使用托盘模式。",
  ...
  GET_URL_TITLE_DESC:
    ...
    "s://iframely.server" +
    ".crestify.com/iframely?url=</code> 来获取页面的标题。",
  ...
  DEBUGMODE_DESC:
    "我建议在启用/禁用该项后重新启动 Obsidian。这将在控制台中启用调试消息，有助于排查问题。" +
    `如果您在使用插件时遇到问题，请启用该项，重现问题，并在 <a href="${URLs.GITHUB_COM_ZSVICZIAN_OBSIDIAN_EXCALIDRAW_PLUGIN_ISSUES}">GitHub</a> 上提出的问题中包含控制台日志。`,
  ...
  MATHJAX_DESC:
    "如果您在绘图中使用 LaTeX，插件需要从服务器获取并加载一个 javascript 库。" +
