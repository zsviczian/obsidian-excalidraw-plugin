...
export default {
  ...
  AI_PROVIDER_PROFILE_MODAL_API_KEY_DESC: "保存在提供商配置中，供其关联的所有模型使用。",
  ...
  AI_MODEL_CONFIG_MODAL_ENDPOINT_DESC: "独立的完整接口地址覆盖。留空则由所选提供商配置推导。",
  ...
  ES_LOCKED_READING_MODE_DESC: "开启后，与 MD-Embeddable 交互时不会自动切换到编辑模式。可用于待办列表和习惯追踪。",
  ...
  ES_PROPERTIES_VISIBLE_DESC: "在嵌入文件顶部显示笔记属性（YAML frontmatter）。",
  ES_PROPERTIES_VISIBLE_WARNING: "全局 Obsidian 设置已隐藏笔记属性。请先更改为“显示”或“源码”。",
...
`,
  ...
  EXTRAS_GATEWAY_DESC: "“{component}”功能已移动至配套插件 Excalidraw Extras，以保持插件精简，避免超出 Obsidian Sync 的大小限制，并让您更明确地控制所需的扩展库及高权限功能。",
  EXTRAS_GATEWAY_INSTALL_BTN: "从社区插件安装",
  EXTRAS_GATEWAY_MANUAL_ENABLE_NOTICE: "我原本实现了根据需要自动启停 Excalidraw Extras 的工作流，但因被 Obsidian 代码扫描器标记为高风险项而被迫移除，尽管我认为相关代码被误判了。目前您需要手动启用 Excalidraw Extras。如果您希望未来有更便捷的体验，请考虑向 Obsidian 官方建议，为安全的自动化工作流提供支持。",
  ...
  EXTRAS_GATEWAY_FEATURE_TIMER_EXPIRED: "Excalidraw Extras: {component} 计时器到期，功能已禁用。",
  EXTRAS_GATEWAY_API_MISSING: "Excalidraw Extras 已启用，但 API 加载失败。请更新该插件。",
  EXTRAS_GATEWAY_FEATURE_TITLE: "功能已禁用",
  EXTRAS_GATEWAY_FEATURE_DESC: "“{component}”功能目前在您的 Excalidraw Extras 设置中处于禁用状态。",
  EXTRAS_GATEWAY_IGNORE_SESSION: "在本次会话忽略",
  EXTRAS_GATEWAY_UPDATE_EXACT: "需要更新 Excalidraw Extras。{component} 功能需要版本 = v{reqVersion} (当前为 v{currentVersion})。",
  EXTRAS_GATEWAY_UPDATE_MIN: "需要更新 Excalidraw Extras。{component} 功能需要版本 >= v{reqVersion} (当前为 v{currentVersion})。",
