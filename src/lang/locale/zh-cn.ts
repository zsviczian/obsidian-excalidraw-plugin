...
export default {
  ...
  BACKUP_DELETE : "删除备份" ,
  BACKUP_CANCEL : "取消" ,
  CACHE_NOT_READY : "抱歉给您带来了不便，加载文件时发生了错误。<br><br><mark>耐心一点可以为您节省大量时间……</mark><br><br>插件有一个备份缓存，但看起来您刚刚启动了 Obsidian。初始化备份缓存可能需要一些时间，通常取决于设备性能，也许需要一分钟或更长时间。当缓存初始化完成时，您将会在右上角收到通知。<br><br>请点击“确定”以重试加载文件，并检查缓存是否已经初始化完成。如果在这条消息后面看到一个完全空白的文件，建议您等待备份缓存准备好后再继续操作。或者，您可以选择“取消”手动修复您的文件。<br>" ,
