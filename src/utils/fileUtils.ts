...
} from "./nestedDependencyTraversal";
...
export interface NestedFileDependencyGraph extends DependencyGraph<TFile> {
  nodes: Map<string, DependencyGraphNode<TFile>>;
}

export interface NestedFileDependencyGraphContext
  extends NestedFileDependencyGraph {
  includeImages: boolean;
  parsedExcalidrawPaths: Set<string>;
}

export const createNestedFileDependencyGraphContext = (
  includeImages = false,
): NestedFileDependencyGraphContext => ({
  includeImages,
  nodes: new Map(),
  parsedExcalidrawPaths: new Set(),
});

/**
 * Builds a path-keyed dependency graph from Obsidian's metadata cache.
 * A context may be shared by related queries so each Excalidraw file is parsed
 * once. The context must remain scoped to one operation to avoid stale edges.
 */
export function getNestedFileDependencyGraph(
  plugin: ExcalidrawPlugin,
  rootFile: TFile,
  includeImages = false,
  sharedContext?: NestedFileDependencyGraphContext,
): NestedFileDependencyGraph {
  const context =
    sharedContext?.includeImages === includeImages
      ? sharedContext
      : createNestedFileDependencyGraphContext(includeImages);
  const pending = [rootFile];

  while (pending.length > 0) {
    const file = pending.pop();
    if (!file || context.parsedExcalidrawPaths.has(file.path)) {
      continue;
    }
    context.parsedExcalidrawPaths.add(file.path);

    const uniqueChildren = new Map<string, TFile>();
    const cache = plugin.app.metadataCache.getFileCache(file);
    if (!cache?.links) {
      context.nodes.set(file.path, { value: file, childKeys: [] });
      continue;
    }
    ...
    if (cache.headings) {
      const embeddedFilesHeadingIndex = cache.headings.findIndex(
        (heading) => heading.heading.toLowerCase() === "embedded files",
      );
      if (embeddedFilesHeadingIndex !== -1) {
        const embeddedFilesHeading = cache.headings[embeddedFilesHeadingIndex];
        startLine = embeddedFilesHeading.position.start.line;
        for (
          let index = embeddedFilesHeadingIndex + 1;
          index < cache.headings.length;
          index++
        ) {
          if (cache.headings[index].level <= embeddedFilesHeading.level) {
            endLine = cache.headings[index].position.start.line;
            break;
          }
        }
      }
    ...
    if (startLine !== -1) {
      for (const link of cache.links) {
        if (
          ...
          (usingCommentFallback && link.position.start.col <= 12)
        ) {
          continue;
        }
        ...
        if (plugin.isExcalidrawFile(linkedFile)) {
          ...
          uniqueChildren.set(linkedFile.path, linkedFile);
          if (!context.nodes.has(linkedFile.path)) {
            context.nodes.set(linkedFile.path, {
              value: linkedFile,
              childKeys: [],
            });
          }
    ...
    context.nodes.set(file.path, {
      value: file,
      childKeys: Array.from(uniqueChildren.keys()),
    });
  }
...
export const getExcalidrawEmbeddedFilesFiletree = (
  ...
  const graph = getNestedFileDependencyGraph(plugin, sourceFile, true);
  return collectReachableDependencyValues(graph, sourceFile.path);
};
...
export const hasExcalidrawEmbeddedImagesTreeChanged = (
  ...
  const graph = getNestedFileDependencyGraph(
    plugin,
    sourceFile,
    true,
    sharedContext,
  );
  return hasReachableDependency(
    graph,
    sourceFile.path,
    (file) => file.stat.mtime > mtime,
  );
};
