...
export async function captureScreenshot(
  ...
  if (options.selectedOnly) {
    ea.copyViewElementsToEAforEditing(
      view.getViewElements().filter((el) => !includedElementIDs.has(el.id)),
    );
  ...
  // Save and set state
  const saveState = () => {
    const { scrollX, scrollY, zoom, viewModeEnabled, linkOpacity, theme } =
      api.getAppState();
    return {
      scrollX,
      scrollY,
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        const scrollX =
          boundingBox.topX -
          margin / options.zoom +
          (col * tileWidth) / options.zoom;
        const scrollY =
          boundingBox.topY -
          margin / options.zoom +
          (row * tileHeight) / options.zoom;
        ...
        const captureWidth =
          col === cols - 1
            ? adjustedTotalWidth - tileWidth * (cols - 1)
            : tileWidth;
        const captureHeight =
          row === rows - 1
            ? adjustedTotalHeight - tileHeight * (rows - 1)
            : tileHeight;
