...
export const carveOutImage = async (
  ...
  const newImage = targetEA.getElement(
    viewImageEl.id,
  ...
  const { folderpath, filename } = await getCropFileNameAndFolder(
    sourceEA.plugin,
    sourceEA.targetView.file.path,
    fname,
  );

  const file = await createImageCropperFile(
    targetEA,
    newImage.id,
    imageLink,
    folderpath,
  ...
  const sourceImageEl = sourceEA.getElement(
    viewImageEl.id,
  ) as Mutable<ExcalidrawImageElement>;
  sourceImageEl.isDeleted = true;

  const replacingImageID = await sourceEA.addImage(
    sourceImageEl.x,
    sourceImageEl.y,
    file,
    true,
  );
  const replacingImage = sourceEA.getElement(
    replacingImageID,
  ) as Mutable<ExcalidrawImageElement>;
...
export const carveOutPDF = async (
  sourceEA: ExcalidrawAutomate,
  embeddableEl: ExcalidrawEmbeddableElement,
...
export const createImageCropperFile = async (
  targetEA: ExcalidrawAutomate,
  imageID: string,
  imageLink: string,
  foldername: string,
  filename: string,
  ...
  const frameID = targetEA.addFrame(
    0,
    0,
    width,
    height,
    "Adjust frame to crop image. Add elements for mask: White shows, Black hides.",
  );
