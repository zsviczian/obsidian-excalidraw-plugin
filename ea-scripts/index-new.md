...
I would love to include your contribution in the script library. If you have a script of your own that you would like to share with the community, please open a [PR](https://github.com/zsviczian/obsidian-excalidraw-plugin/pulls) on GitHub. Add the script and its icon/preview, add or update its entry in `ea-scripts/script-store.json`, then run `npm run script-store:build`. The build regenerates this legacy catalog and only advances `directory-info.json` mtimes for script/icon files that actually changed.
---
