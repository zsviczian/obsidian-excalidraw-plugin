...
(() => {
  ...
  // src/adapter/nativeStagingIsolation.ts
  var withIsolatedNativeStaging = (ea2, prepareCommit) => {
    const previousElements = ea2.elementsDict;
    if (!previousElements) return prepareCommit(() => true);
    const previousImages = ea2.imagesDict;
    const elements = {};
    const images = {};
    ea2.elementsDict = elements;
    if (previousImages) ea2.imagesDict = images;
    try {
      return prepareCommit(
        () => ea2.elementsDict === elements && (!previousImages || ea2.imagesDict === images)
      );
    } finally {
      if (ea2.elementsDict === elements) ea2.elementsDict = previousElements;
      if (previousImages && ea2.imagesDict === images) ea2.imagesDict = previousImages;
    }
  };
  ...
  var hasExplicitTargetViewProperty = (ea2) => {
    return Object.hasOwn(ea2, "targetView");
  };
  ...
  var applyElementPatchesViaLegacyEditing = async (ea2, patch, canExecute) => {
    const { copyViewElementsToEAforEditing, getElement, addElementsToView } = ea2;
    ...
    try {
      const commit = withIsolatedNativeStaging(ea2, (ownsStaging) => {
        copyViewElementsToEAforEditing.call(ea2, targets);
        if (!canExecute() || !ownsStaging()) return false;
        const editableById = /* @__PURE__ */ new Map();
        for (const elementPatch of patch.elementPatches) {
          const editable = getElement.call(ea2, elementPatch.id);
        ...
        if (!canExecute() || !ownsStaging()) return false;
        return addElementsToView.call(ea2, false, false);
      });
  ...
  var applyPatchViaUpdateScene = (ea2, patch, canExecute) => {
    ensureTargetView(ea2);
  ...
  var applyElementPatchesViaUpdateScene = (ea2, patch, canExecute) => {
    return applyPatchViaUpdateScene(ea2, patch, canExecute);
  };
  var applyElementPatches = async (ea2, patch, canExecute) => {
    ...
    if (hasLegacyElementMutationCapabilities(ea2)) {
      const legacyApplied = await applyElementPatchesViaLegacyEditing(ea2, patch, canExecute);
    ...
    if (hasUpdateSceneCapability(ea2)) {
      return {
        ok: applyElementPatchesViaUpdateScene(ea2, patch, canExecute)
      };
    }
    return {
      ok: false
  ...
  var applyPatch = async (ea2, patch, canExecute = () => true) => {
    const expired = {
      status: "capabilityMissing",
    ...
    if (patch.reorder) {
      const patchApplied = applyPatchViaUpdateScene(ea2, patch, canExecute);
      if (!canExecute()) return expired;
      if (!patchApplied) {
        return {
          status: "preflightFailed",
      ...
      const elementApply = await applyElementPatches(ea2, patch, canExecute);
      if (!canExecute()) return expired;
      if (!elementApply.ok) {
        return {
          status: "preflightFailed",
  ...
  // ../../node_modules/xstate/dist/raise-8dd8aaec.esm.js
  var Mailbox = class {
...
${err2.message}`);
  ...
  // ../../node_modules/xstate/dist/assign-54f9e2d3.esm.js
  ...
  // ../../node_modules/xstate/dist/StateMachine-28a9f50e.esm.js
...
${err2.message}`);
  ...
  // ../../node_modules/xstate/dist/log-66075555.esm.js
  ...
  var executeIntentRequest = async (ea2, planner, canExecute) => {
    ...
    while (attempts < 2) {
      ...
      const applyOutcome = await applyPatch(ea2, planned.value, canExecute);
      ...
      runMutation: fromPromise(
        async ({
          ...
          if (input.request.type === "APPLY_REQUEST") {
            const outcome2 = input.request.canExecute() ? await applyPatch(input.ea, input.request.patch, input.request.canExecute) : { status: "capabilityMissing", reason: "Scene ownership changed before execution." };
            return {
              kind: "apply",
...
${lines.join("\n")}`);
  ...
  var ownerKey = /* @__PURE__ */ Symbol.for("excalidraw-layer-manager.sidepanel-lifecycle-owner");
  var getOwner = (tab) => Reflect.get(tab, ownerKey);
  var hasSidepanelLifecycleOwner = (tab) => {
    return getOwner(tab) !== void 0;
  };
  ...
  var createRuntimeSidepanelLifecycleBinding = (input) => {
    ...
    const sync = () => {
      ...
      let closed = false;
      const owns = () => !disposed && !closed && boundTab === tab && getOwner(tab) === binding;
  ...
  // src/runtime/sidepanelPendingCreationOwnership.ts
  var registryKey = /* @__PURE__ */ Symbol.for("excalidraw-layer-manager.pending-sidepanel-creations.v1");
  var createSidepanelPendingCreationLease = (host) => {
    const scope = host.plugin ?? host.app ?? globalThis;
    const key = host.activeScript || host;
    const token = /* @__PURE__ */ Symbol("pending-sidepanel-invocation");
    let disposed = false;
    const readRegistry = () => Reflect.get(scope, registryKey);
    const group = () => {
      ...
      if (!entry) {
        entry = { pending: /* @__PURE__ */ new Set(), orphans: /* @__PURE__ */ new Map() };
    ...
    const flush = () => {
      ...
      const ownsEntry = () => readRegistry() === registry && registry.get(key) === entry;
  ...
  // src/ui/sidepanel/rename/inlineRenameDomSession.ts
  var containsNode = (nodes, nodeId) => nodes.some((node) => node.id === nodeId || containsNode(node.children, nodeId));
  var InlineRenameDomSession = class {
    #generation = 0;
    #authority = null;
    #input = null;
    #selection = null;
    ...
    owns(generation, host) {
      return generation === this.#generation && this.hasAuthority(host);
    }
    hasAuthority(host) {
      return !!this.#authority && this.#authority.view === host.targetView && this.#authority.api === host.getExcalidrawAPI?.();
    }
    beforeRender(host, nodeId, tree, contentRoot) {
      this.#generation += 1;
  ...
  var SidepanelHostSelectionBridge = class {
    ...
    mirrorSelectionToHost(elementIds, expectedSceneBinding) {
      ...
      const hasAuthority = () => mirrorRequestId === this.#latestMirrorRequestId && targetView === this.#host.targetView && leafView === view?.leaf?.view && api === this.resolveFallbackApi(expectedSceneBinding);
  ...
  var ExcalidrawSidepanelRenderer = class {
    ...
    #inlineRenameDomSession = new InlineRenameDomSession();
    #dragDropController;
    #settingsWriteQueue;
    ...
    renderNodes(container, nodes, visibleNodes, currentSelectedNodes, depth, selectedNodeIds, actions, branchContext, matchKindByNodeId) {
      ...
      renderSidepanelRowTree({
        ...
        visitNode: ({
          ...
          const canHandleRename = () => !this.#disposed && this.#inlineRenameDomSession.owns(renameGeneration, this.#host) && !!this.#contentRoot?.contains(row);
          ...
          const { row, renameInputForAutofocus } = renderSidepanelRow({
            ...
            isInlineRenameActiveForNode: (targetNodeId) => canHandleRename() && this.#inlineRenameController.nodeId === targetNodeId,
  ...
  var haveEquivalentSnapshots = (left, right) => {
    ...
    return left.sceneBinding.refreshKey === right.sceneBinding.refreshKey && left.currentTargetView === right.currentTargetView && (!right.hasExplicitTargetViewProperty || left.sceneApi === right.sceneApi) && left.targetViewIdentity === right.targetViewIdentity && left.targetViewUsable === right.targetViewUsable && left.hostEligible === right.hostEligible && left.hasCachedTargetView === right.hasCachedTargetView && left.cachedTargetViewIdentity === right.cachedTargetViewIdentity;
  };
  ...
  var createLayerManagerRuntime = (ea2, providedRenderer) => {
    ...
    const reconcileHostContext = (signal) => {
      ...
      if (previousBindingKey !== result.snapshot.sceneBinding.sceneKey || hostContextSnapshot.currentTargetView !== result.snapshot.currentTargetView || hostContextSnapshot.hasExplicitTargetViewProperty && hostContextSnapshot.sceneApi !== result.snapshot.sceneApi) {
        hostAuthorityEpoch += 1;
        clearPendingReadiness();
        clearSceneChangeSubscription();
      }
    ...
    const recoverReleasedView = () => {
      ...
      let relevant = false;
      try {
        const leaf = releasedContext.leaf;
        const workspace = releasedContext.workspace;
        const replacementPath = leaf?.view?.file?.path;
        relevant = !!leaf && (workspace?.activeLeaf === leaf || workspace?.getMostRecentLeaf?.() === leaf) && (!releasedContext.filePath || !replacementPath || replacementPath === releasedContext.filePath);
      } catch {
      }
      if (!relevant) {
        clearPendingRecovery();
    ...
    const shouldScheduleHostContextRefresh = (input) => {
      return input.result.changed || input.result.rebound || input.previousRefreshKey !== input.result.snapshot.sceneBinding.refreshKey;
    };
    ...
    const scheduleApiReadinessCheck = () => {
      if (disposed || readinessTimer !== null) return;
      const expectedView = ea2.targetView;
      ...
      if (!ea2.getExcalidrawAPI && !("excalidrawAPI" in view)) return;
      ...
      readinessTimer = setTimeout(() => {
        ...
        if (ea2.targetView !== expectedView) {
          clearPendingReadiness();
          reconcileHostContext("manual");
          scheduleHostContextRefresh();
          return;
    ...
    const captureMutationAuthority = () => {
      ...
      const leafView = view?.leaf?.view;
      return () => !disposed && !hostFocusReleased && epoch === hostAuthorityEpoch && targetView === ea2.targetView && api === view?.excalidrawAPI && leafView === view?.leaf?.view;
    };
