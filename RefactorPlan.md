...
| Clarify persistence lifecycle names | Complete | Renamed only private state, request fields, parameters, and locals in the persistence/synchronization hot path: save requests now express own-write suppression, forced persistence, and same-file guard bypass; prepared serialization, TextFileView load identity, reload fallback, incoming data, merged elements, metadata application, and reload file IDs have explicit names. `executeSaveRequest()` replaces the ambiguous private save-policy method name. Internal autozoom callers use `suppressAutozoomOnce()`, while the historical public `preventAutozoom()` remains as a compatibility delegate. `ViewSemaphores.preventReload`, every timer, save argument position/default, await boundary, side-effect policy, and branch condition remain unchanged pending the later ownership refactors. Focused manual validation passed |
| Introduce semantic persistence/lifecycle accessors | Complete | `ViewSaveCoordinator` now exposes `isSaveInProgress` and `isBusy` while retaining the existing semaphore-backed definitions. `ExcalidrawView` exposes lifecycle, same-file edit, save-state, and own-write suppression operations. `FileManager`, `EventManager`, and `MarkdownImageEditor` no longer inspect or mutate `ViewSemaphores` directly. Each replacement preserves the former Boolean expression exactly; `EventManager` deliberately uses the narrower `isClosing()` query rather than adding migration rejection, and `isSaveInProgress` remains broad until persistence and synchronization ownership split in the next behavioral checkpoint. Focused manual validation passed |
| Split persistence and synchronization ownership | Complete | `ViewSaveCoordinator` is now the sole internal reader/writer of the compatibility `saving` field and brackets each queued persistence request itself. `synchronizeWithData()` uses a separate view-local `isSynchronizing` state. Existing mutual exclusion, busy checks, reload/unload waiting, and the three-second dropped-sync policy remain in place using the two explicit states. The synchronization-only `setDirty()` rejection is removed, so a merged scene that preserves newer local content can always advance the document revision and remain dirty for autosave/flush. Focused manual validation passed; the pending-latest synchronization improvement remains a separate checkpoint |
...
| --- | --- | --- | --- |
| 2026-09-12 | Closed the persistence/synchronization ownership checkpoint | Retained the implemented save-versus-sync state split and reconciliation dirty correction without further source changes | User confirmed the focused collision, reconciliation, multi-view, and lifecycle behavior works as expected |
| 2026-09-11 | Split save-in-progress ownership from incoming synchronization | Moved all production reads/writes of `ViewSemaphores.saving` into `ViewSaveCoordinator`, introduced independent view-local synchronization state, and retained mutual exclusion at the coordinator/view boundary. Reload, unload, force-save, and Markdown-image busy behavior still consider both operations where the former broad flag did. Removed the deterministic synchronization-time dirty guard; revision coalescing and the existing three-second dropped-sync behavior are otherwise unchanged | Node 22.22.2 production and library builds pass with the established 33 circular-dependency warnings; bundle syntax and `git diff --check` pass. Focused ESLint is clean for the coordinator and types, while `ExcalidrawView.ts` retains exactly its established 18 errors. The production bundle is 4,901,420 bytes. Focused manual validation remains: older incoming element versus newer local element from a clean baseline, ordinary incoming-newer synchronization without an autosave loop, save/sync collision, same-file multi-view viewport retention, and close/migration during synchronization |
| 2026-09-11 | Closed the semantic persistence/lifecycle accessor checkpoint | Retained the implemented accessors and consumer migrations without further source changes | User reported no regressions after the focused manual validation |
| 2026-09-11 | Added semantic accessors ahead of persistence-state extraction | Added coordinator-owned save-state queries and view-owned lifecycle/same-file/own-write operations, then replaced every direct semaphore reference in `FileManager`, `EventManager`, and `MarkdownImageEditor`. Storage, timeout ownership, save queue state, method ordering, and all former conditions remain unchanged. No new manager or persisted state was introduced | Node 22.22.2 production and library builds pass with the established 33 circular-dependency warnings; bundle syntax, unused-symbol lint, direct-semaphore-reference searches in all three migrated consumers, and `git diff --check` pass. Focused ESLint is clean outside `ExcalidrawView.ts`, which retains exactly its established 18 errors. The production bundle is 4,900,225 bytes, 342,655 bytes below 5 MiB. Focused manual validation remains: own-write modify consumption, same-file Markdown-image switching/flush, active-leaf save behavior, and close/migration rejection |
| 2026-09-11 | Closed the naming-only persistence lifecycle checkpoint | Retained the implemented semantic names and compatibility boundary without further source changes | User confirmed all recommended focused manual tests completed successfully |
| 2026-09-11 | Completed the naming-only persistence lifecycle checkpoint | Replaced ambiguous private save/synchronization names with the semantic vocabulary from the validated lifecycle review, without moving storage or changing behavior. `ViewSemaphores.preventReload` deliberately remains until content-aware expected-write suppression replaces it; the public `preventAutozoom()` name remains as a compatibility delegate. Updated the lifecycle design document to match the implemented names. No manager, timer, revision rule, persistence branch, export policy, or release-note entry was added | Node 22.22.2 production and library builds pass with the established 33 circular-dependency warnings; `node --check dist/main.js`, unused-symbol lint, repository searches for every retired private name, and `git diff --check` pass. Focused ESLint is clean for `FileManager.ts` and `ViewSaveCoordinator.ts`; `ExcalidrawView.ts` retains exactly its established 18 errors, and full source lint retains exactly the established 120-error/0-warning backlog. The production bundle is 4,899,704 bytes, 343,176 bytes below 5 MiB. Focused manual validation remains: positional save-policy routing, own-write echo suppression, external synchronization/viewport retention, and main↔popout dirty migration |
