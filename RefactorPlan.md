...
| Migrate the settings tab to declarative settings with legacy fallback | Checkpoint 4E complete | Checkpoints 4A-4D established the shared shell and migrated every non-AI section. Checkpoint 4E adds AI enablement, session usage, diagnostics, provider/model management, and token limits to the same `SettingsPageModel` hierarchy. Independent controls are searchable and exported separately, while the interdependent provider/profile/model editor remains one shared integrated component so provider renames and removals continue updating model references atomically. Manual validation confirmed the AI settings, live installed-script refresh, and legacy PDF/Markdown cross-links. The Obsidian dependency/minimum remains `1.8.7` |
