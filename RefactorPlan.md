...
| Establish a typed Excalidraw/Obsidian host boundary | In progress; render-canvas limits validated | The Excalidraw fork now contains a documented, versioned `ObsidianCommonHostAdapter` contract and window-runtime registry with idempotent, stale-disposer-safe teardown. `getAreaLimit()` and `getWidthHeightLimit()` are the first production consumers: each evaluated main/popout runtime receives a package-manager-owned adapter, reads the plugin's current `ExcalidrawConfig` values, and releases the registration when that window package is deleted. The legacy lookup remains as a transitional fallback for older published artifacts and unmigrated capabilities. The UI-mode vocabulary is now canonicalized on Excalidraw's `StylesPanelMode`; the plugin preserves `UIMode` as a type-only compatibility alias |
