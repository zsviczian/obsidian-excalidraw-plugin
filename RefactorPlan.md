...
| Migrate the settings tab to declarative settings with legacy fallback | Checkpoint 2 complete; canonical adapter pending | Added a local, type-only Obsidian 1.13.0 structural contract and capability-checked runtime facade without augmenting the pinned Obsidian `1.8.7` declarations, activating declarative rendering, or changing the dependency/minimum version |
