...
| Migrate the settings tab to declarative settings with legacy fallback | Checkpoint 4D complete | Checkpoints 4A-4C established the shared shell and migrated every non-AI section through Embed/export and custom pens. Checkpoint 4D adds Fonts, Miscellaneous features, Taskbone OCR, Excalidraw Automate, installed-script settings, and Compatibility to the same `SettingsPageModel` hierarchy. Both layouts share canonical specifications and integrated components, the former standalone renderers are removed, internal setting cross-links receive a scoped, theme-resistant underline/accent treatment, and legacy links open their folded ancestors before scrolling. The declarative utility toolbar and breadcrumbs share one compact centered header. The dynamic installed-script page is guarded against pre-layout access to `ScriptEngine`, and the Obsidian dependency/minimum remains `1.8.7`. AI remains isolated for the final checkpoint-4 batch, 4E |
