...
| Share the main React/Excalidraw runtime with popouts (batch 2 checkpoint 1) | Feasibility and owner-realm behavior validated; ready for PackageManager simplification | Popout leases retain their actual acquisition window for migration/persistence ownership but return the already-evaluated main-window package, avoiding a second React/Excalidraw evaluation. A popout-local `ExcalidrawLib` compatibility alias is installed only while the popout has a live lease and is removed on final release. Upstream's accepted stable `ownerDocument` boundary plus the audited fork-only realm fixes restored pointer drawing, keyboard shortcuts, WYSIWYG, fonts, dialogs, portals, and teardown in popouts. Dirty main↔popout migration, repeated stress migration, and the fresh empty-scene Welcome-font gate passed without data loss or new Excalidraw console errors. The isolated strategy switch and legacy per-window evaluation path remain only to provide a safe rollback boundary for the next checkpoint, which will simplify PackageManager without changing migration/state-loading behavior |
...
| Preserve edits made during persistence with revision/coalescing semantics (Phase 1B.4b) | Complete | `ViewSaveCoordinator` now advances a monotonic current revision from Excalidraw durable store increments and existing explicit dirty calls, records the revision owned by each save request, and advances the saved revision only after persistence, an unchanged terminal result, or the established view-unload scheduled-write handoff. One save loop owns at most one active request and one merged trailing request. Edits during synchronization/compression remain dirty and update that trailing request; an overlapping request already satisfied by the active revision is discarded without reaching the old `save.skipped` path. Manual force-save busy/notice behavior remains compatible. `flush()` waits for the loop and force-persists any revision still dirty before close or migration; the Electron delayed view-unload write remains intact. Runtime validation on SYL exercised a manual save followed by edits during compression, continued editing across successive trailing saves, final convergence to the newest revision, immediate close/reopen, and the inline `[[` regression. Serialization snapshots, side-effect policies, loader behavior, and popout runtime ownership are unchanged |
...
| Ignore inline suggestion portals in the modal-save observer | Complete | `ObserverManager` now saves a dirty active drawing only when the direct body child added to the active view document is an actual `.modal-container`. The inline link suggester's body-portaled `.suggestion-container` no longer masquerades as Settings or the command palette and cannot trigger synchronization/reload while a text element is being edited. Real modal-open persistence, outside-canvas clicks, file menus, leaf changes, window blur, autosave, and view teardown retain their existing save paths. Runtime validation confirmed that `[[` opened and remained usable after duplicating an equation without changing the save counter, while the next real command-palette modal triggered the intended save and durable reopen restored the complete scene |
...
| 2026-08-25 | Reverted the Phase 1C migration prototype before preparing the Phase 1A/1B PR | Preserved committed 1C.1/1C.2 on the original `performance-refactor` branch, stashed the rejected plugin 1C.3 bitmap handoff and all sibling-fork bitmap changes under explicit names, and created the delivery branch at the validated end of Phase 1B. The sibling fork source is clean and master-equivalent, and its clean Obsidian artifact replaced the experimental local runtime. The handoff folder remains untracked and outside the delivery | This avoids carrying the prototype's 17-second destination `addFiles()` pause, approximately 2.19 GB heap peak, and source-window teardown freezes into the PR. The Phase 1A/1B commits and independent repository-zip script remain in history. Future work should begin on a separate branch with the persistent-main-runtime feasibility spike documented above |
