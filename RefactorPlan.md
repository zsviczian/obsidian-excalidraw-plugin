...
| Migrate the settings tab to declarative settings with legacy fallback | Checkpoint 4A complete | Extracted the canonical row specifications, legacy renderer, declarative converter, typed binding registry, and focus-safe coalesced persistence queue. The persistence diagnosis and recovery correction are complete and manually validated. Checkpoint 4A now stages compact Basic and Saving navigation pages with descriptions on folded declarative navigation entries and opened declarative pages; legacy mode displays each description only once, before its fold. One `SettingsPageModel` tree owns the Basic/Saving hierarchy, descriptions, leaf definition builders, and legacy leaf renderers: both modes therefore use Updates and startup, Files and folders, Stencil Library, Storage and autosave, and Filename with Automation scripts inside Files and folders. The shared compact Copy settings/NotebookLM/help/support toolbar renders in both modes, while Mastery remains root-only; legacy mode additionally keeps its custom search. On Obsidian 1.13+, a persisted top-level preference defaults to the searchable declarative interface and can select the legacy single-page interface for the next Obsidian session; older versions always use legacy and do not show the preference. Markdown export recursively traverses declarations and places the control type before the description. The remaining sections stay intentionally absent during this partial declarative test and will be added in 4B-4E. The dependency/minimum version remains `1.8.7` |
