...
| Migrate the settings tab to declarative settings with legacy fallback | Checkpoint 4B complete | Checkpoint 4A established the shared shell, persistence bridge, Basic/Saving pages, layout preference, toolbar, and recursive Markdown export. Checkpoint 4B extends the same `SettingsPageModel` tree to Excalidraw appearance and behavior plus Links, transclusion and TODOs. Both modes now share logical Editing and previews, UI modes, Hotkeys, Theme, Zoom and pan, Pen, Grid, Laser, modifier-key, link behavior/opening/appearance, TODO, and transclusion groups. Manual validation covered the 4B settings UI and drove shared fixes for complex-component width, initial disabled state, and settings-triggered viewport preservation. The remaining sections stay intentionally absent from the declarative test interface until 4C-4E. The dependency/minimum version remains `1.8.7` |
