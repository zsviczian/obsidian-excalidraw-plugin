...
| Close body-portaled floating UI with its Obsidian tab | Implemented; awaiting manual validation | The fork's `ObsidianRadixPortal` now observes the source container only while floating content is open and asks each dropdown, properties popover, or tool popover to clear its canonical open state when the container loses its layout box. Body-portaled content can no longer outlive a hidden workspace leaf and jump to the viewport origin |
...
| --- | --- | --- | --- |
| 2026-09-04 | Closed body-portaled Excalidraw menus when Obsidian hides their source tab | Traced the top-left jump to the fork-only `ObsidianRadixPortal`: Obsidian removes an inactive leaf from layout, but the portal remains under the owning document body, leaving Radix open without a measurable anchor. Added open-scoped ancestor mutation and container resize observation in the portal and routed its one-shot hidden notification to the existing canonical close paths for dropdown menus, properties/color/font popovers, and local tool popovers. The plugin active-leaf/save lifecycle, host-adapter protocol, public API, persisted scene/settings data, ordinary in-container `Popover.tsx`, and upstream portal behavior remain unchanged | Node 22.22.2. The focused portal and legacy Popover suites pass (4 tests); the portal test proves visible ancestor class changes do not close content and an actual lost layout box closes exactly once. The fork's `yarn build:obsidian` passes and emits declarations plus all four artifacts. JSON ESLint output has no finding on changed lines; the standard formatter remains blocked by the repository's known ESM/CJS formatter mismatch and reports only established findings elsewhere in the touched files. Full `yarn test:typecheck` retains 311 established test-harness errors (`window.h`, disabled `createTestHook`, stale fixtures) with zero diagnostics in touched files; the DropdownMenu suite is likewise blocked before collecting tests by the disabled `createTestHook`. After copying only the four generated artifacts into the installed package, plugin production and development builds pass with the established 33 circular-dependency warnings and development sourcemap warning; bundle syntax and focused `Messages.ts` lint pass, and the 4,877,900-byte production `dist/main.js` remains 364,980 bytes below 5 MiB. Full plugin lint retains its established 123-error/0-warning backlog with no finding in the touched source file. Focused Obsidian runtime validation remains pending. Highest-risk manual check: open each tray/main menu, canvas/shape/color/font properties popover, and selection/freedraw tool popover, switch to a sibling tab, and confirm immediate closure with no top-left flash or focus theft; repeat in a main window and popout. Also verify click-outside, Escape, theme/canvas-color changes while open, and reopen behavior. Desktop is required; one narrow/mobile smoke test is warranted because resize observation deliberately closes a portal whose source loses all layout boxes |
