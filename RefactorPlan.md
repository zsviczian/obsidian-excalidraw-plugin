...
| Establish a typed Excalidraw/Obsidian host boundary | In progress; desktop UI-mode checkpoint validated | The Excalidraw fork now contains a documented, versioned `ObsidianCommonHostAdapter` contract and window-runtime registry with idempotent, stale-disposer-safe teardown. The render-canvas limits, device-info getter, and desktop UI-mode getter are migrated and user-validated. `getDesktopUIMode()` obtains and runtime-normalizes its value through the adapter without touching global `app`; the legacy lookup remains only when no host is registered, preserving compatibility with older published artifacts. The UI-mode vocabulary is canonicalized on Excalidraw's `StylesPanelMode`; the plugin preserves `UIMode` as a type-only compatibility alias |
