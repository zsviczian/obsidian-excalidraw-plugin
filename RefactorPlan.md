...
| Migrate the settings tab to declarative settings with legacy fallback | Checkpoint 4C complete | Checkpoints 4A-4B established the shared shell and migrated Basic, Saving, appearance/behavior, Links, TODOs, and transclusions. Checkpoint 4C extends the same `SettingsPageModel` tree to Embed/export, image caching, PDF defaults/import, interactive and image-rendered Markdown embeds, rendering multiplier, and custom pens. Both layouts share canonical specifications, dependency callbacks, integrated components, descriptions, and grouping; the former standalone legacy renderers are removed. Declarative pages also have guarded breadcrumbs and working internal cross-references without changing the `1.8.7` dependency/minimum. Fonts, Experimental, Excalidraw Automate, and Compatibility remain for 4D; AI remains isolated for 4E |
