...
| Migrate the settings tab to declarative settings with legacy fallback | Checkpoint 3 complete; conversion batches pending | Extracted the canonical row specifications, legacy renderer, declarative converter, and typed binding/persistence registry. All 125 existing canonical rows now exercise the shared legacy infrastructure, while `getSettingDefinitions()` remains explicitly empty and the dependency/minimum version remains `1.8.7` |
