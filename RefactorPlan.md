...
| Migrate the settings tab to declarative settings with legacy fallback | Checkpoints 5/6 complete; final cleanup in progress | Checkpoints 4A-4E migrated the full settings surface and passed focused manual validation. The combined checkpoint 5/6 audit confirms the complete definition tree is active on Obsidian 1.13+ behind the persisted, restart-applied `useDeclarativeSettings` preference; returning an empty array remains the intentional legacy fallback for older Obsidian versions and users who select the single-page layout. All supported locales have unique sibling page paths, maintained locale page metadata is complete, integrated components carry targeted aliases, and utility/promotion declarations are excluded from filtered search. Manual validation confirmed ordinary and integrated search results, deep navigation and breadcrumbs, and restart switching in both directions. The Obsidian dependency/minimum remains `1.8.7` |
