...
| Performance baseline diagnostics (Phase 0) | Baseline captured; focused diagnostic extension awaiting rerun | The first four runtime captures confirmed expensive large-scene save synchronization/serialization, repeated mostly-no-op `loadSceneFiles()` requests, costly warm image-cache reconstruction, full image reload during popout migration, and the `CanvasNodeFactory.removeNode()` map-key mismatch. The temporary flat-text diagnostics now also tag every view-scoped scene-file request with a stable `reason=`, expose unresolved/missing scene-file counts and retry scheduling/execution, break warm-cache work into IndexedDB/dependency/materialization stages, and provide a manual console checkpoint for before/after forced-GC measurements. No performance behavior has been changed in this checkpoint |
...
| In-memory popout migration handoff (Phase 1C) | Reverted and parked | The drawing-state handoff registry and ImageBitmap prototype were removed from the Phase 1A/1B delivery branch. Although state parsing and destination runtime initialization were fast, publishing transferred bitmaps through the newly created destination runtime blocked for roughly 17 seconds and drove heap use to roughly 2.19 GB; repeated MOC EA moves could freeze Obsidian after Electron destroyed the source window. The rejected work remains recoverable in local archival branch/stashes, but no fork API or plugin runtime dependency from that experiment is part of this checkpoint. A future branch should first test a persistent main React runtime with fresh roots and small drawings before returning to MOC EA migration |
