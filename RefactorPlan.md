...
| Establish a typed Excalidraw/Obsidian host boundary | In progress; device-info checkpoint validated | The Excalidraw fork now contains a documented, versioned `ObsidianCommonHostAdapter` contract and window-runtime registry with idempotent, stale-disposer-safe teardown. The render-canvas limits and device-info getter are migrated and user-validated. `getObsidianDeviceInfo()` returns the registered adapter's device capabilities without touching global `app`; the legacy lookup remains only when no host is registered, preserving compatibility with older published artifacts. The UI-mode vocabulary is canonicalized on Excalidraw's `StylesPanelMode`; the plugin preserves `UIMode` as a type-only compatibility alias |
