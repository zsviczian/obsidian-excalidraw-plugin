...
| Migrate the settings tab to declarative settings with legacy fallback | Checkpoint 3 complete; checkpoint 4 remains on hold after persistence recovery correction | Extracted the canonical row specifications, legacy renderer, declarative converter, typed binding registry, and focus-safe coalesced persistence queue. Diagnosis traced the former loss risk to unbounded/cross-caller writes rather than normal termination of one isolated save. The production path uses one global serialized stable-snapshot writer plus one device-local, vault-namespaced IndexedDB last-known-good record. Every valid load refreshes recovery; invalid synchronized runtime data is rejected and immediately repaired from active memory; invalid startup data is restored from recovery and written back automatically. A genuinely new installation (missing file and no recovery) initializes silently. Missing data with recovery asks whether to restore the backup or factory-reset; corrupt data without recovery asks whether to factory-reset or wait for an externally restored file. Rendering the help/promotion surface no longer causes a save. Checkpoint 4 remains paused for focused manual validation and explicit maintainer resumption. All 125 existing canonical rows still use the shared legacy infrastructure, `getSettingDefinitions()` remains explicitly empty, and the dependency/minimum version remains `1.8.7` |
...
| 4A. Convert shell, help/promotion, General, and Saving | On hold by maintainer | The persistence diagnosis and production root fix are complete, but do not resume this batch until the focused manual validations above pass and the maintainer explicitly resumes checkpoint 4. Then define the tab shell and top utilities, preserve all help/promotion links, and migrate the first related settings batch through both adapters |
