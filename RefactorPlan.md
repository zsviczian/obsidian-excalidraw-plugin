...
| Stabilize known Excalidraw-file classification across metadata-cache refresh | Reproduced; pending separate correctness checkpoint | Same-tab back navigation can begin loading a parent drawing less than one millisecond after its embedded child drawing finishes saving. During that interval Obsidian's metadata cache can temporarily lack the child's frontmatter, so `PluginFileManager.isExcalidrawFile()` classifies the already-known child as ordinary Markdown and the parent renders a Markdown embeddable until reopened. This path uses the long-standing `forceSaveIfRequired()` navigation save rather than the Phase 1B blur policy. The likely narrow fix is to use the manager's existing `excalidrawFiles` set as a fallback for previously confirmed files while the metadata cache refreshes; validate intentional frontmatter removal before adopting it |
...
| 2026-08-24 | Validated the Phase 1A.3 loader checkpoint on the prior failure scene and the representative regression scene | The first capture, despite its `large-terminal` checkpoint label, was the 46-file representative drawing: it emitted all 46 files and completed normally in 9.05 seconds. Its subsequent command-save request started with no active loader and completed in 1.6 ms, demonstrating that completion released the loader and later work could advance. The later capture, despite its `normal` label, was the prior 169-file failure scene: it emitted all 169 files in five batches and completed normally in 7.51 seconds. The new diagnostic split identified zero missing vault files and three external/local entries, explaining why the missing-vault retry path was not applicable | All three `sceneFiles.runStart` records have matching `sceneFiles.runComplete terminalState=completed` records; there is no `sceneFiles.loaderStalled`, the 46-file view closed cleanly to zero Canvas nodes, and the maintainer reported no unfiltered errors. This validates the normal terminal path, guards against false timeouts on both workloads, and shows the historical scene no longer leaves the manager wedged. Because every task settled, this run did not force the 30-second watchdog or exercise a missing-vault-file retry; those branches retain structural/build validation and should be targeted if an actual stalled or missing-vault case recurs rather than manufacturing another broad manual test now |
