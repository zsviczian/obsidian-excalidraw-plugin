...
| Compress per-window React payload | Complete; superseded by the shared bundled runtime | React, ReactDOM, and the official JSX runtimes were previously inflated before plugin bootstrap for private popout-window package creation. The later shared-runtime architecture removed the need for retained/evaluated React source, and the current build bundles those modules normally |
...
| Bundle the shared React runtime normally | Implemented; awaiting focused runtime validation | Removed the nested React-runtime Rollup build, compressed `REACT_PACKAGES` bootstrap, regex postprocessing, and their obsolete source files/dependency. The main plugin now imports and bundles React, legacy/client ReactDOM, and both JSX runtimes once; the production `jsxDEV` fallback is preserved. The library build and separately built Excalidraw artifact still externalize React so no second copy crosses either boundary. `PackageManager` continues to own the shared package, host adapters, leases, and popout aliases; no view, migration, or persistence lifecycle changed. The obsolete `ownerDocument` compatibility cast in `ExcalidrawRoot` was removed because the published component contract now declares the prop directly. This packaging change was implemented in commit `39346559ced5aa349388d22954fa9a73a51f2959`. If the production bundle later approaches the 5 MiB limit, restoring that commit's compressed React packaging is a practical fallback and recovers approximately 114,871 bytes |
