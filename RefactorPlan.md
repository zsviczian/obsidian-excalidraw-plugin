...
| Establish a typed Excalidraw/Obsidian host boundary | In progress; common layer validated | The versioned `ObsidianCommonHostAdapter` now serves every modern `commonObsidianUtils.ts` capability: render-canvas limits, device info, desktop/preferred UI modes, and highlight color. Registered main/popout runtimes do not use global `app` for these paths; legacy lookup remains only for older published artifacts with no registered host. The cached plugin is deliberately not retired yet because the separate higher-level `packages/excalidraw/obsidianUtils.ts` bridge still consumes it. The UI-mode vocabulary is canonicalized on Excalidraw's `StylesPanelMode`; the plugin preserves `UIMode` as a type-only compatibility alias |
