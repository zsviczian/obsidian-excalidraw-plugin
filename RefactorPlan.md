...
| Migrate the settings tab to declarative settings with legacy fallback | Checkpoint 3 complete; checkpoint 4 remains on hold after persistence root fix | Extracted the canonical row specifications, legacy renderer, declarative converter, typed binding registry, and focus-safe coalesced persistence queue. Diagnosis traced the former loss risk to unbounded/cross-caller writes rather than normal termination of one isolated save. The production path now uses one global serialized stable-snapshot writer without recovery authority, rejects invalid synchronized loads before they can replace active settings, and no longer saves merely because the help/promotion surface was rendered. Checkpoint 4 remains paused for focused manual validation and explicit maintainer resumption. All 125 existing canonical rows still use the shared legacy infrastructure, `getSettingDefinitions()` remains explicitly empty, and the dependency/minimum version remains `1.8.7` |
